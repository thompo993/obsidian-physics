...
For Resnet 18, the standard input image is identical to that defined above in section \ref{CORRECT SECTION HERE}. This allowed the same data loaders to be used. Resnet 18 has 17 convolutional layers, and to tune it to this specific model the classifier head was adjusted to accommodate tumor classification. The classifier head was adjusted to take the output of the final residual layer, and then apply a linear layer, followed by a ReLU layer and a final layer to make the models outputs 3 logits associated with the tumor class. Dropout was set to 0.5, which is higher than the baseline cnn
