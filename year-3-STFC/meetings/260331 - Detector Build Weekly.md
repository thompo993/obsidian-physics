---
tags:
  - meeting
created: 2026-08-13
---
# Links: 

# Agenda: 

# Minutes:
### Tile Testing Results:
- 1 Tile definitely to discard
- 2 within error margins,
- 6 within error threshold range and error.
- 27 tiles found total, 1 discarded.
- Tiles below Id048 - 30mm seem good, get 6 tiles from James
### Metal Lapping Jig:
- 30mm metal delivered
- 105 and 210 in R12 - actually being treated as priority
- No set date
- Grade 2 titanium screws not yet been ordered - expensive
- We seem to be lagging behind schedule
- MuSR Rending done by tomorrow Absolute latest

### Plots: 
![[fig-260806-onenote-transfer-det-weekly-260331-1.png]]
![[fig-260806-onenote-transfer-det-weekly-260331-2.png]]
![[fig-260806-onenote-transfer-det-weekly-260331-3.png]]